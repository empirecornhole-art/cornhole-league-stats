...
const eventStatColumns = [
  { label: "Rank", keys: ["Rank", "RANK", "Ranking"], decimals: 0 },
  { label: "PPR", keys: ["PPR", "ppr"], decimals: 2 },
  { label: "Rounds", keys: ["Rounds", "ROUNDS", "Round", "rounds"], decimals: 0 },
  { label: "Points", keys: ["Points", "POINTS", "points"], decimals: 0 },
  { label: "OPPR", keys: ["OPPR", "Opp PPR", "Opponent PPR"], decimals: 2 },
  { label: "Opp Pts", keys: ["Opp Pts", "Opponent Points", "OPP Points"], decimals: 0 },
  { label: "DPR", keys: ["DPR", "dpr"], decimals: 2 },
  { label: "4 Baggers", keys: ["4 Baggers", "Four Baggers"], decimals: 0 },
];

function findEventStatForStanding(row: any, eventStats: any[]) {
  const player = getPlayer(row).toLowerCase();
  const week = getWeek(row).toLowerCase();
  const type = getType(row).toLowerCase();
  return eventStats.find((stat) => {
    return (
      getPlayer(stat).toLowerCase() === player &&
      getWeek(stat).toLowerCase() === week &&
      getType(stat).toLowerCase() === type
    );
  });
}
...
function summarizeEventStats(rows: any[]) {
  ...
  const avg = (keys: string[]) => {
    const values = validRows
      .map((row) => getStatNumber(row, keys))
  ...
  const sum = (keys: string[]) => {
    const total = validRows.reduce((acc, row) => acc + getStatNumber(row, keys), 0);
    return total ? formatValue(total, 0) : "-";
  };

  return {
    players: validRows.length,
    totalRounds: sum(["Rounds", "ROUNDS", "rounds"]),
    totalPoints: sum(["Points", "POINTS", "points"]),
    avgPPR: avg(["PPR", "ppr"]),
    avgOPPR: avg(["OPPR", "Opp PPR", "Opponent PPR"]),
    avgDPR: avg(["DPR", "dpr"]),
    totalOppPoints: sum(["Opp Pts", "Opponent Points", "OPP Points"]),
    totalFourBaggers: sum(["4 Baggers", "Four Baggers"]),
  };
}
...
export default function LeagueClient() {
  ...
  const weekStandingsRows = useMemo(() => {
  ...
  const weeks = useMemo(() => {
    return Array.from(new Set(weekStandingsRows.map(getWeek).filter(Boolean))).sort(weekSort);
  }, [weekStandingsRows]);
  ...
  const visibleWeekStandingsRows = weekStandingsRows.filter((row) => {
    return (!week || getWeek(row) === week) && (player === "All Players" || getPlayer(row) === player);
  });

  const visibleEventStatsRows = eventStats.filter((row) => {
    return (
  ...
  const playerSeasonStats = useMemo(() => {
    ...
    return (
      (data?.stats || []).find(
        (row) => getPlayer(row) === selectedProfilePlayer && (!profileSeason || getSeason(row, profileSeason) === profileSeason)
      ) || null
    );
  }, [data, selectedProfilePlayer, profileSeason]);

  const playerWeeklyStandingsRows = useMemo(() => {
  ...
  const groupedPlayerWeeks = useMemo(() => {
    ...
    for (const row of playerWeeklyStandingsRows) {
        ...
        <div className="rounded-2xl border border-neutral-800 bg-[#141414] p-4 shadow-xl">
          <div className="flex flex-col gap-3 md:flex-row md:items-end">
            <div>
              ...
              <select className="block rounded-lg border border-neutral-700 bg-[#242424] p-2 text-white" value={season} onChange={(e) => { setSeason(e.target.value); setProfileSeason(e.target.value); }}>
      ...
      <section className="mx-auto max-w-7xl space-y-6 p-4">
              ...
              <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
                <div className="h-96">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={standings.slice(0, 12)} margin={{ top: 24, right: 10, bottom: 75, left: 0 }}>
                      <XAxis dataKey="name" interval={0} angle={-28} textAnchor="end" height={95} tick={{ fontSize: 12 }} />
                      <YAxis />
                      <Tooltip />
        ...
        {tab === "weeks" && (
          <Card title="Weekly Results">
            ...
            <h3 className="mb-2 text-lg font-black text-[#f04a22]">Standings With Player Stats</h3>
            <StandingWithStatsTable rows={visibleWeekStandingsRows} eventStats={eventStats} />
            <h3 className="mb-2 mt-8 text-lg font-black text-[#f04a22]">Event Totals / Averages</h3>
              ...
              {!selectedProfilePlayer ? <p className="text-neutral-400">Select a player to view stats, seasons, weeks, and event breakdowns.</p> : (
                <div className="space-y-6">
                  <div className="flex flex-wrap gap-3">
                    <select className="rounded-lg bg-[#242424] p-2" value={profileSeason} onChange={(e) => { setProfileSeason(e.target.value); setProfileWeek("All Weeks"); }}>{seasons.map((s) => <option key={s}>{s}</option>)}</select>
                    ...
                    {groupedPlayerWeeks.length === 0 ? <p className="text-neutral-400">No weekly records found for this selection.</p> : groupedPlayerWeeks.map(([weekLabel, byType]) => (
                      <div key={weekLabel} className="rounded-xl border border-neutral-800 bg-[#1c1c1c] p-4">
                        ...
                        {Object.entries(byType).map(([eventType, rows]) => {
                          const statsRows = rows.map((r) => findEventStatForStanding(r, eventStats)).filter(Boolean);
                          return <div key={eventType} className="mb-4"><div className="mb-2 inline-block rounded-full bg-[#f04a22] px-3 py-1 text-xs font-black uppercase">{eventType}</div><StandingWithStatsTable rows={rows} eventStats={eventStats} compact /><EventSummary rows={statsRows} /></div>;
                        })}
                      </div>
                    ))}
                  </div>
...
function StandingWithStatsTable({ rows, eventStats, compact = false }: { rows: any[]; eventStats: any[]; compact?: boolean }) {
  return <div className="overflow-x-auto"><table className="w-full text-[12px]"><thead><tr className="text-left text-neutral-400"><th className="p-2">Rank</th><th className="p-2">Player</th><th className="p-2">Team</th><th className="p-2">Standing Pts</th>{eventStatColumns.slice(1).map((col) => <th key={col.label} className="p-2">{col.label}</th>)}</tr></thead><tbody>{rows.map((row, index) => { const stat = findEventStatForStanding(row, eventStats); return <tr key={index} className="border-t border-neutral-800"><td className="p-2">{formatValue(row.Rank ?? row.RANK, 0)}</td><td className="p-2 font-bold text-[#f04a22]">{getPlayer(row)}</td><td className="p-2">{clean(row.Team || row.TEAM) || "-"}</td><td className="p-2">{formatValue(row.Points ?? row.POINTS, 0)}</td>{eventStatColumns.slice(1).map((col) => <td key={col.label} className="p-2">{stat ? formatValue(getStatValue(stat, col.keys), col.decimals) : "-"}</td>)}</tr>; })}</tbody></table></div>;
}

function EventSummary({ rows }: { rows: any[] }) {
  const summary = summarizeEventStats(rows);
  return <div className="mt-4 rounded-xl border border-neutral-800 bg-[#202020] p-4"><div className="grid gap-3 sm:grid-cols-2 md:grid-cols-4"><MiniStat label="Players" value={summary.players} /><MiniStat label="Total Rounds" value={summary.totalRounds} /><MiniStat label="Total Points" value={summary.totalPoints} /><MiniStat label="Avg PPR" value={summary.avgPPR} /><MiniStat label="Avg OPPR" value={summary.avgOPPR} /><MiniStat label="Avg DPR" value={summary.avgDPR} /><MiniStat label="Opp Points" value={summary.totalOppPoints} /><MiniStat label="4 Baggers" value={summary.totalFourBaggers} /></div></div>;
}
